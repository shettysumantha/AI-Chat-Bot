...
BEGIN
    ...
    WHERE kc.user_id = p_user_id AND LOWER(kc.title) = LOWER(p_title);
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION fn_save_uploaded_document(
...
BEGIN
    RETURN QUERY
    SELECT kd.id, kd.file_name, kd.file_path, kd.file_size, kd.file_type, kd.status, kd.uploaded_at
...
BEGIN
    RETURN QUERY
    SELECT km.id, km.role, km.message, km.created_at
    FROM kb_chat_messages km
    WHERE km.user_id = p_user_id AND km.conversation_id = p_conversation_id
...
RETURNS VOID AS $$
BEGIN
    INSERT INTO kb_chat_messages(conversation_id, user_id, role, message, created_at)
    VALUES (p_conversation_id, p_user_id, p_role, p_message, NOW());
END;
$$ LANGUAGE plpgsql;
